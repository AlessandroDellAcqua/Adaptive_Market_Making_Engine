"""Run a reproducible Monte Carlo comparison and write compact artifacts."""
...
def main() -> None:
    ...
    manifest = write_experiment_manifest(
        ...
        experiment_name="intensity_strategy_comparison",
    ...
    with (args.output_dir / "run_metrics.csv").open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=sorted(rows[0]))
        writer.writeheader()
        writer.writerows(rows)
    write_comparison_svg(representative, args.output_dir / "representative_comparison.svg")
